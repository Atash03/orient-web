...
function AdvertisementImage({ advertisements, intervalTime = 5000, width = 2000, height = 200 }: Props) {
    const [index, setIndex] = useState<number>(0);

    useEffect(() => {
        const timer = setInterval(() => setIndex(prev => (prev + 1) % advertisements.adds.length), intervalTime);
    ...
    return (
        <Link href={currentItem.url} target='_blank'>
            <Image src={currentItem.media} alt={currentItem.title} height={height} width={width} className='object-cover' />
        </Link>
    )
}
